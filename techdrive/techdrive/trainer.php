<?php 
      ...
      {		
          ...
          $insert = "INSERT INTO trainer( rno,fname,lname,gender,dob,sno,adrs,ano,email) VALUES ( '$rno','$fname','$lname','$gender', '$dob','$sno','$adrs','$ano','$email')";
    ...
    if(isset($_POST['edit'])) // when click on Update button
      {
        ...
        $edit = mysqli_query($con,"update trainer set rno='$rno', fname='$fname' ,lname = '$lname',gender = '$gender',dob = '$dob',sno = '$sno',adrs = '$adrs',ano = '$ano', email = '$email' where sid = $sid"); 
        
          if($edit)
          {
              mysqli_close($con); // Close connection
              header("location:trainer.php"); // redirects to all records page
              exit;
          }
          else
          {
    ...
    if(isset($_POST['delete'])) // when click on Update button
      {
        $sid = $_POST['sid'];
        $del = mysqli_query($con,"delete from trainer where sid = $sid"); // delete query 
        if($del)
        {
            mysqli_close($con); // Close connection
            header("location:trainer.php"); // redirects to all records page
    ...
    if($rno>0)
      {
        $query=mysqli_query($con,"SELECT * FROM  trainer WHERE rno=$rno");
